    ...
    public function retrieve(Request $request){
      ...
      if(sizeof($result) > 0){
        ...
        foreach ($result as $key) {
          ...
          // $this->response['data'][$i]['created_at_human'] = Carbon::createFromFormat($this->dateTimeFormat, $result[$i]['created_at'])->copy()->tz($this->response['timezone'])->format('F j, Y h:i A');
