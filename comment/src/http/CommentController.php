    ...
    public function retrieveCommentsWithImages(Request $request){
      ...
      if(sizeof($result) > 0){
        ...
        foreach ($result as $key) {
          ...
          $this->response['data'][$i]['amen'] = app('App\Http\Controllers\EducationController')->retrieveWithPayload('comment_id', '=', $result[$i]['id'], 'reaction', '=', 'amen');
          $this->response['data'][$i]['love'] = app('App\Http\Controllers\EducationController')->retrieveWithPayload('comment_id', '=', $result[$i]['id'], 'reaction', '=', 'love');
          $i++;
        }
      }
      return $this->response();
